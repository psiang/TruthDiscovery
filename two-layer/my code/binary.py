...
def pre_assign(votes, ground_truths):
    ...
    for v in votes:
        if query.get(v[0], "Null") != "Null" and worker.get(v[1], "Null") != "Null":
            replay[worker[v[1]]][query[v[0]]] += category[v[2]]
...
# twolayer加噪
def perturb_twolayer(answers, pr, style):
    nw = len(answers)
    nq = len(answers[0])
    answers_perturbed = copy.deepcopy(answers)
    for i in range(nw):
...
def td(answers):
    ...
    for time in range(100):
        ...
        for i in range(nw):
            ...
            error.append(er)
        for i in range(nw):
            temp = 1.0 * error[i] / max(error)
            if temp == 0:
                temp += 0.0000000001
            weight[i] = - math.log(temp)
...
def execute_style(votes, ground_list):
    ...
    for style in style_list:
        ...
        for e in e_list:
            ...
            for i in range(n):
                answers_perturbed = perturb_twolayer(answers, pr, style)
...
# 比较不同分布对两层TD的影响
def execute(votes, ground_list, style):
    n = 100
    answers, ground_truth = pre_assign(votes, ground_list)
